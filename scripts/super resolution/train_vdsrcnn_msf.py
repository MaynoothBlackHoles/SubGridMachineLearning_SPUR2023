...
for i in range(EPOCHS):
    ...
    nf.vdsr_test_loop(dataset["single sf data"][0], model, loss_fn, device, sf2_PSNR)
    nf.vdsr_test_loop(dataset["single sf data"][1], model, loss_fn, device, sf4_PSNR)
    nf.vdsr_test_loop(dataset["single sf data"][2], model, loss_fn, device, sf8_PSNR)
