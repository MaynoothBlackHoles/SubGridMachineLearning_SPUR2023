...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 13 13:54:01 2023
...
num_densities = []
for ds in datasets:
    n = (ds.all_data()[("gas", "number_density")]).value
...
density_dir = '/raw_number_density'
...
H2_fractions = []
for ds in datasets:
    n = ds.all_data()[('PartType0', 'H2_fraction')].value
    H2_fractions.append(n.flatten())
    
    
    
#%%
H2_dir = '/raw_H2_fraction'
...
dynamic_times = []
for ds in datasets:
    ct = ds.all_data()[('CoolTime')].value * ds.time_unit.to('s').value
    dt = np.sqrt(3 * np.pi / (32 * G * ds.all_data()[('density')])).value
...
#%%
time_scales_dir = '/raw_time_scales'
